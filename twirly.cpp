...
void process_image(PNG_Canvas_BW& image, string filename, int rmax = -1, int xc = -1, int yc = -1, int alpha = 45){
	width = image.get_width();
	height = image.get_height();
    if(rmax<0)  rmax = width/2 + height/2;
    if(xc < 0)  xc = width/2;
    if(yc<0)    yc = height/2; 
    PNG_Canvas_BW output_image(width, height); 
    TwirlyTransform twirly(rmax, xc, yc, alpha);
    int x,y;
